    ...
    <Fragment>
          ...
          <div className="d-flex flex-column column-schedules">
            ...
            {iexIds &&
              iexIds.map((id) => (
                <div
                  key={"iexId-item" + id}
                  className="schedule-item bg-light border border-white btn btn-light btn-sm p-0"
                >
                  {id}
                </div>
              ))}
            ...
            {activities &&
              activities.map((activity) => (
                <div
                  key={"validator-empty-" + activity}
                  className="schedule-item border border-white bg-white btn btn-white btn-sm p-0"
                ></div>
              ))}
          </div>
          <div className="d-flex flex-column column-schedules">
            ...
            {iexIds &&
              iexIds.map((id) => (
                <div
                  key={"agent-item-" + id}
                  className="schedule-item border border-white btn btn-light btn-sm p-0"
                >
                  {schedules[id].name}
                </div>
              ))}
            ...
            {activities &&
              activities.map((activity) => (
                <div
                  ...
                  className={
                    activity === "OFF"
                      ? "schedule-item border border-white bg-warning"
                      : /[0-9]+:[0-9]+/.test(activity)
                      ? "schedule-item border border-white bg-light"
                      : "schedule-item border border-white text-light bg-danger"
                  }
                >
                ...
                >
                    ...
                    iexIds.map((id) => {
                      ...
                      if (daily) {
                        if (daily.output) {
                          if (daily.output === "OFF") {
                            return (
                              <button
                                key={"item-" + date + "-" + id}
                                className="schedule-item border border-white btn btn-warning btn-sm p-0"
                                onClick={() =>
                                  handleSelectDetail(
                                    daily.breakdown,
                                    schedules[id].name,
                            ...
                            return (
                              <button
                                key={"item-" + date + "-" + id}
                                className="schedule-item border border-white btn btn-danger btn-sm p-0"
                                onClick={() =>
                                  handleSelectDetail(
                                    daily.breakdown,
                                    schedules[id].name,
                              ...
                              >
                                {daily.output.length < 20
                                  ? daily.output
                                  : daily.output.substring(0, 20)}
                              </button>
                            )
                          }
                        }
                        ...
                        return (
                          <div
                            key={"item-" + date + "-" + id}
                            className="schedule-item border border-white bg-danger text-light px-1"
                          >
                            OUT
                          </div>
                        )
                  ...
                  {activities &&
                    activities.map((activity) => {
                      ...
                      return (
                        <div
                          key={"validator-" + activity + "-" + date}
                          className={"schedule-item border px-1 border-white "}
      ...
      {detail && (
        <div className="mt-3 text-center">
          ...
          {detail.breakdown ? (
            detail.breakdown.map((detail, index) => (
